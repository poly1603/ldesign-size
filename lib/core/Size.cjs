...
class Size {
  ...
  constructor(input = 0, rootFontSize = DEFAULT_FONT_SIZE) {
    this._flags = 0;
    this._value = index.parseSizeInput(input);
   ...
   */
  toPixels() {
    return index.convertSize(this._value, "px", this._rootFontSize);
  }
  /**
   * Convert to rem
   */
  toRem() {
    return index.convertSize(this._value, "rem", this._rootFontSize);
  }
  /**
   * Convert to em
   */
  toEm() {
    return index.convertSize(this._value, "em", this._rootFontSize);
  }
  /**
   * Convert to viewport width
   */
  toViewportWidth() {
    return index.convertSize(this._value, "vw", this._rootFontSize);
  }
  /**
   * Convert to viewport height
   */
  toViewportHeight() {
    return index.convertSize(this._value, "vh", this._rootFontSize);
  }
  /**
   * Convert to percentage
   */
  toPercentage() {
    return index.convertSize(this._value, "%", this._rootFontSize);
  }
  /**
   * Convert to specific unit
   */
  to(unit) {
    return index.convertSize(this._value, unit, this._rootFontSize);
  }
  /**
   * Convert to string
   */
  toString() {
    return index.formatSize(this._value);
  }
  /**
   ...
   */
  scale(factor) {
    const scaled = index.scaleSize(this._value, factor);
  ...
  add(other) {
    ...
    const result = index.addSizes(this._value, otherSize._value, this._rootFontSize);
    if (this._isPooled) {
      otherSize.dispose();
    }
  ...
  subtract(other) {
    const otherSize = this._isPooled ? SizePool.getInstance().acquire(other, this._rootFontSize) : new Size(other, this._rootFontSize);
    const result = index.subtractSizes(this._value, otherSize._value, this._rootFontSize);
    if (this._isPooled) {
      otherSize.dispose();
    }
    return this._isPooled ? SizePool.getInstance().acquire(result, this._rootFontSize) : new Size(result, this._rootFontSize);
   ...
   */
  round(precision = 2) {
    const rounded = index.roundSize(this._value, precision);
   ...
   */
  clamp(min, max) {
    const clamped = index.clampSize(this._value, min, max, this._rootFontSize);
