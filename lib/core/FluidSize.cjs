...
class FluidSizeCalculator {
  constructor() {
    ...
    this.viewport = DeviceDetector.getDeviceDetector().getViewport();
    this.lastViewport = {
      ...this.viewport
    };
    this.unsubscribe = DeviceDetector.getDeviceDetector().onChange((viewport) => {
      if (!this.isDestroyed) {
  ...
  createResponsiveSize(config) {
    ...
    const parsed = index.parseSizeInput(selectedSize);
    return index.formatSize(parsed);
  }
  /**
...
exports.FluidSizeCalculator = FluidSizeCalculator;
exports.fluid = fluid;
exports.fluidTypographyPresets = fluidTypographyPresets;
exports.getFluidSizeCalculator = getFluidSizeCalculator;
exports.modularScaleRatios = modularScaleRatios;
/*! End of @ldesign/size | Powered by @ldesign/builder */
//# sourceMappingURL=FluidSize.cjs.map
